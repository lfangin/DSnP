...
DBAppendCmd::exec(const string& option)
{
  ...
  size_t s = (doRow)? dbtbl.nCols() :dbtbl.nRows();
  vector<int> tmp;
  int n;
  for(size_t i= 1;i < s+1 ;i++  ){
    if(i > options.size()-1) tmp.push_back(INT_MAX);
    else if(options[i]== "-") tmp.push_back(INT_MAX);
    else if(doRow && !myStr2Int(options[i],n)) return CMD_EXEC_ERROR;
    else if(!doRow && !myStr2Int(options[i],n)) return CMD_EXEC_ERROR;
    else tmp.push_back(n);
  }
  DBRow r;
  if(doRow){
    for(int i=0;i<tmp.size();i++) r.addData(tmp[i]);
    dbtbl.addRow(r);
    r.reset();
  }
  else dbtbl.addCol(tmp);
...
DBPrintCmd::exec(const string& option)
{
  ...
  else if (options.size()==0) return errorOption(CMD_OPT_MISSING,"");
  else {
    if(myStr2Int(options[0],id_row)){
      if(!checkRowIdx(options[0],id_row)) return CMD_EXEC_ERROR;
      //if(id_row > dbtbl.nRows()||id_row < 0)
      else if(options.size() > 2) return errorOption(CMD_OPT_EXTRA,options[2]);
      else {
        int id_col ;
        if(!checkColIdx(options[1],id_col)) return CMD_EXEC_ERROR;
        else if (dbtbl.getData(id_row,id_col)==INT_MAX) cout << "null" <<endl;
        else cout << dbtbl.getData(id_row,id_col) << endl;
        return CMD_EXEC_DONE;
      }
    }
    else if(myStrNCmp("-Table",options[0],2)==0){ cout << dbtbl ; return CMD_EXEC_DONE;}
    else if(myStrNCmp("-Summary",options[0],2)==0){ dbtbl.printSummary(); return CMD_EXEC_DONE;}
    else if(options.size()!=2) return CMD_EXEC_ERROR;
    }
  bool doRow = false;
  if (myStrNCmp("-Row", options[0], 2) == 0) doRow = true;
  else if (myStrNCmp("-Column", options[0], 2) != 0)
...
DBPrintCmd::usage(ostream& os) const
{
   os << "DBPrint < (int rowIdx) (int colIdx)\n"
      << "        | -Row (int rowIdx) | -Column (colIdx) | -Table | -Summery >" << endl;
}
