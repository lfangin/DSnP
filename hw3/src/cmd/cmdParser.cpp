...
CmdParser::parseCmd(string& option)
{
   ...
   CmdExec* e = getCmd(tok);
   if(!e) cerr << "Illegal command!!(" << tok <<")"<< endl;
   else if(pos != string::npos) option = str.substr(pos);
...
CmdParser::listCmd(const string& str)
{
  ...
  pos = str.find_first_not_of(del,0);
  while(pos!= string::npos){
    new_pos = str.find_first_of(del,pos);
    if(new_pos != string::npos) pos = new_pos +1;
    else pos = new_pos;
    ncount++ ;
  }
  ...
  if(ncount > 1){
    for(CmdMap::const_iterator it = _cmdMap.begin(); it!= _cmdMap.end();++it){
      string cmp = (it->first)+(it->second->getOptCmd());
      if (token.size()>cmp.size()) break;
      else if(myStrNCmp(cmp,token,cmp.size())==0){
        cout << '\n';
  ...
  if(ncount == 1){
    for(CmdMap::const_iterator it = _cmdMap.begin(); it!= _cmdMap.end();++it){
      string cmp = (it->first)+(it->second->getOptCmd());
      if (token.size()>cmp.size()) break;
      else if(myStrNCmp(cmp,token,token.size())==0) tmp.push_back(cmp);
    }
    if(tmp.size() > 1){
      cout << '\n';
      ...
      string s = tmp[0].substr(token.size());
