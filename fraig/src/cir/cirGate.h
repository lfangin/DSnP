...
public:
   ...
   void set_inDFS()const{inDFS = true;}
   bool get_inDFS(){return inDFS;}
   void reset_inDFS(){inDFS = false;}
...
class AIGgate : public CirGate
{
  ...
  void printGate(int& count) const{
    if(flag) return;
    if(!_faninList.empty()){
      //cout << "AIG not empty faninlist with "<<_faninList.size()<<endl;
...
class POgate : public CirGate
{
  ...
  void printGate(int& count) const{
    if(!_faninList.empty()){
      //cout << "PO not empty faninlist with "<<_faninList.size()<<endl;
      for(size_t i = 0;i!= _faninList.size();i++)
        getfanin(i) -> printGate(count);
    }
