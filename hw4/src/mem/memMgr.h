...
#define toSizeT(t)  (t % SIZE_T ? (t/SIZE_T+1) * SIZE_T : t)
        // TODO
//
// To demote 't' to the nearest multiple of SIZE_T
// e.g. Let SIZE_T = 8;  downtoSizeT(9) = 8, downtoSizeT(100) = 96
#define downtoSizeT(t)  (t%SIZE_T ? (t/SIZE_T)*SIZE_T : t)  // TODO
...
class MemBlock
{
   ...
   bool getMem(size_t t, T*& ret) {
      // TODO
      if(getRemainSize() < toSizeT(t)) return false;
      ret = (T*) _ptr;
      _ptr += toSizeT(t);
      return true;
   }
...
class MemRecycleList
{
   ...
   void reset() {
      ...
      _first = NULL;
      _nextList = NULL;
      //how can I know if it is occupied by MemMgr errrrrrrr
...
private:
   ...
   MemRecycleList<T>* getMemRecycleList(size_t n) {
      ...
      while(1){
        if(ptr->getArrSize() == n) return ptr;
        else if (ptr -> getNextList()!= 0) ptr = ptr -> getNextList();
        else {
          MemRecycleList<T>* ptr_next = new MemRecycleList<T> (n);
          ptr -> setNextList(ptr_next);
          //ptr -> getNextList();
          return ptr_next;
        }
      }
   ...
   T* getMem(size_t t) {
      ...
      if(ret == 0){
        if(!(_activeBlock -> getMem(t,ret))){
          ...
          MemBlock<T>* tmp = new MemBlock<T>(_activeBlock,_blockSize);
          _activeBlock = tmp;
